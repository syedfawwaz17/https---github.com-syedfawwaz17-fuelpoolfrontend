import api from "./api";
import { z } from "zod";
import { UserDto, getMe } from "./auth"; // Assuming getMe can fetch user by ID
 ...
 * @param userId The ID of the user (the reviewee).
...
export async function getReviewsForUser(userId: string): Promise<Review[]> {
  try {
    ...
    return reviewsWithReviewerData.filter((review): review is Review => review !== null);
